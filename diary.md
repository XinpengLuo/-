...
4. 出来的话 首先将first 指向是第几个报数的 然后 需要一个辅助指针 事先指向最后一个节点 这里的最后节点是相对于**first**而言的 即永远指向first的前一个节点  当报数的时候 两个指针同时向前移动m - 1 次  出圈的时候 first再向前移动一次 helper当前节点的next指向新的first![rary/Application Support/typora-user-images/image-20201130210045210.png)
...
3. 如果此最右节点的右指针不为空 并且指向了自己 则将右指针指向变为空 **同时指向当前节点的指针指向右子树** 
...
哈哈这是好久没有写题解了 感觉已经没必要了 现在上手直接冲了！

写这道题的原因是这道题是我这几个月来收获最大的一道题 必须记录下关于位运算 前缀和的！

**首先思考要让一个区间字符串中每个字符出现的次数都是偶数需要满足什么？ 假设此字符串位s[i,j]**（题目是全为偶数以及最多只能有一个奇数，最多一种奇数的情况一会儿再说！）

答案：`对于字符串s，他的子串s[0, i - 1]以及子串s[0, j]这两个字符串中各个字符出现的次数可以不同，但是每一个字符出现的次数必须同奇同偶！`

及：假设前面个字符串a出现了2次 后面个字符串可以出现2次 4次 6 次。。。。

因为 `偶数 - 偶数 = 偶数 奇数 - 奇数 = 奇数` 如果前面有个字符串和当前字符串同奇偶性，那么他们相减，中间的字符串必然满足全部为偶数！

所以对于这种情况我们很自然的想到了用空间换时间的情况，当我们遍历到字符串`s[0,j]`的时候，我们在遍历j之前，有多少次状态满足各个字符出现的次数的奇偶性和当前字符串的奇偶性是相同的。

一个最笨的方法就是我当前直接去统计每个字符，但是这样你知道每个字符统一一遍，寻找时再判断一遍，空间时间都浪费了！

所以这里引出 **10个二进制位表示10个字符的奇偶状态** 因为我们并不关心他出现了多少次 只关心当前每个字符是奇数还是偶数！

比如`0000011`表示a出现的次数是奇数 b出现的次数是奇数 

**所以当我们要找前面是否有字符串和当前字符串是否同奇偶时，就变成了找前面是否有状态他的二进制表示方式和当前一样。！**

这就很容易想出一边遍历，一边哈希记录即可。

我们再来看只能有一种字符出现的次数为奇数的情况，那其实也不难，假设当前状态为`10001101`我们依次改变每一位的状态，看看前面是否有对应的数字出现过，就代表除了这个数字外其他数字的奇偶性质都是一样的！

对于到代码层面，不用真的统计出现多少次，遍历到当前字符时，如果之前对应的状态是0则变成1 1 变成0， 所以位运算异或即可。

同时改变状态也同理， 获取要改变字符的位置，1左移动，异或即可。
...
public class Solution {
    ...
    public long better(String word){
        long res = 0;
        int state = 0;
        int[] map = new int[1024]; //因为只有10个字符 每个字符有两种情况 故一共2的10次方 1024
        map[0] = 1; //全部都没有出现 则全都是偶数 +1 比如"aa" 状态为00000000 如果没有初始化设置 那么会少的
        for(int i = 0; i < word.length(); i++){
            state = state ^ (1 << (word.charAt(i) - 'a')); //将上一轮状态中对应的字符的奇偶性质进行反转就是当前的心状态
            res += map[state];
            for(int j = 0; j < 10; j++){
                int temp = state ^ (1 << j); //依次改变每一位状态 枚举出来
                res += map[temp];
            }
            map[state]++;
        }
